...
import (
	...
	"net/http"
	"strconv"
	"sxshop/order_web/api"
	"sxshop/order_web/forms"
	"sxshop/order_web/global"
	"sxshop/order_web/models"
...
func New(ctx *gin.Context) {
	...
	_, err = global.OrderSrvClient.UpdateOrderStatus(context.Background(), &proto.OrderStatus{
		OrderSn: "11111111" + uuid.NewV4().String(),
		Status:  string("WAIT_BUYER_PAY"),
	})
	if err != nil {
		zap.S().Errorw("新建订单号失败")
		zap.S().Errorw(err.Error())
		api.HandleGrpcErrorToHttp(err, ctx)
		return
	}
...
func Detail(ctx *gin.Context) {
	...
	//p.NotifyURL = global.ServerConfig.AliPayInfo.NotifyURL
	//p.ReturnURL = global.ServerConfig.AliPayInfo.ReturnURL
	//p.Subject = "随心商城订单-" + rsp.OrderInfo.OrderSn
