...
func BrandList(ctx *gin.Context) {
	...
	pSize := ctx.DefaultQuery("psize", "60")
	pSizeInt, _ := strconv.Atoi(pSize)

	rsp, err := global.GoodsSrvClient.BrandList(context.WithValue(context.Background(), "ginCoontext", ctx), &proto.BrandFilterRequest{
		Pages:       int32(pnInt),
...
func UpdateCategoryBrand(ctx *gin.Context) {
	...
	zap.S().Infof("categoryBrandForm: %v----------------", categoryBrandForm)
	id := ctx.Param("id")
	i, err := strconv.ParseInt(id, 10, 32)
	if err != nil {
		zap.S().Infof("categoryBrandForm: %v----------------", id)
		ctx.Status(http.StatusNotFound)
		return
	}
