...
import (
	...
	"sxshop/goods_web/api"
	"sxshop/goods_web/forms"
...
func New(ctx *gin.Context) {
	...
	//如何设置库存
	//TODO 商品的库存 - 分布式事务
	stockClient := global.InventorySrvClient
	_, err = stockClient.SetInv(context.Background(), &proto.GoodsInvInfo{
		GoodsId: int32(rsp.Id),
		Num:     goodsForm.Stocks,
	})
	if err != nil {
		api.HandleValidatorError(ctx, err)
		zap.S().Infof("设置库存失败, err: %v", err)
		return
	}
...
func Stocks(ctx *gin.Context) {
	id := ctx.Param("id")
	ac, err := strconv.ParseInt(id, 10, 32)
	if err != nil {
		ctx.Status(http.StatusNotFound)
		return
	}
	...
	r, err := stockClient.InvDetail(context.Background(), &proto.GoodsInvInfo{
		GoodsId: int32(ac),
	})
	if err != nil {
		api.HandleValidatorError(ctx, err)
		zap.S().Infof("查询失败, err: %v", err)
		return
	}
	reMap := map[string]interface{}{}

	stock := make([]interface{}, 0)
